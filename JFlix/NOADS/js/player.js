...
const SERVER_URLS = {
  ...
  'vidsrc': 'https://vidsrc.xyz/' // Premium Server 3 (iOS only)
};
...
function loadVideo(id, type, season = null, episode = null) {
  ...
  if (type === 'movie' || type === MEDIA_TYPES.MOVIES) {
    ...
    switch(currentServer) {
      ...
      case '2embed':
        ...
        break;
      case 'vidsrc':
        videoUrl = `${SERVER_URLS[currentServer]}embed/movie?tmdb=${id}`;
        break;
      default:
    ...
    if (season !== null && episode !== null) {
      switch(currentServer) {
        ...
        case '2embed':
          ...
          break;
        case 'vidsrc':
          videoUrl = `${SERVER_URLS[currentServer]}embed/tv?tmdb=${id}&season=${season}&episode=${episode}`;
          break;
        default:
      ...
      switch(currentServer) {
        ...
        case '2embed':
          ...
          break;
        case 'vidsrc':
          videoUrl = `${SERVER_URLS[currentServer]}embed/tv?tmdb=${id}`;
          break;
        default:
  ...
  // Apply sandbox attribute to all servers for ad-blocking, as requested.
  // Note: This may affect playback on some servers.
  videoPlayer.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms allow-presentation allow-orientation-lock allow-pointer-lock allow-top-navigation-by-user-activation');
...
function changeServer(server) {
  ...
  // Validate server (only 'videasy', '2embed', and 'vidsrc' are valid)
  if (server !== 'videasy' && server !== '2embed' && server !== 'vidsrc') {
