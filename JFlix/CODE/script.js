...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayVoucherCode(code, validityDays, payerEmail) { 
        ...
        let copyButton = document.getElementById('copy-voucher-button');
        if (!copyButton && generatedVoucherCodeP && generatedVoucherCodeP.parentNode) { 
            copyButton = document.createElement('button');
            copyButton.id = 'copy-voucher-button';
            copyButton.style.marginLeft = '10px'; 
            // Insert after the voucher code span/paragraph, or adjust as needed for your HTML structure
            // Assuming generatedVoucherCodeP is a span inside a p, or similar
            generatedVoucherCodeP.parentNode.insertBefore(copyButton, generatedVoucherCodeP.nextSibling.nextSibling); 
        }
        if (copyButton) { 
        ...
        let notificationP = document.getElementById('voucher-notification');
        if (!notificationP && voucherResultDiv) { 
            notificationP = document.createElement('p');
            notificationP.id = 'voucher-notification';
            notificationP.style.marginTop = '10px';
            notificationP.style.fontSize = '0.9em';
            voucherResultDiv.appendChild(notificationP);
        }
        if (notificationP) { 
            if (payerEmail) {
                notificationP.innerHTML = `Your voucher code has been generated. A copy has also been sent to <strong>${payerEmail}</strong>.`;
            } else {
                notificationP.innerHTML = `Your voucher code has been generated.`; 
            }
        }

        // Set the voucher code and validity
        if(generatedVoucherCodeP) generatedVoucherCodeP.textContent = code;
        if(voucherValidityDurationSpan) voucherValidityDurationSpan.textContent = validityDays;

        // Display the voucher result div
        if(voucherResultDiv) voucherResultDiv.style.display = 'block';
    }   
    ...
    async function generateVoucher(plan, paymentDetails) {
        ...
        try {
            ...
            if (voucherData.success && voucherData.voucher_code) {
                ...
                displayVoucherCode(voucherData.voucher_code, voucherData.validity_days, payerEmail);
                ...
                if (voucherData.warning) {
                    console.warn('Warning from voucher generation:', voucherData.warning);
                            ...
                            // Attempt to send voucher email via Edge Function
            if (payerEmail) { // Only attempt if payerEmail is available
                try {
                    console.log(`Attempting to call Edge Function 'send-voucher-email' for ${payerEmail}`);
                    const { data: emailSendData, error: emailSendError } = await window.supabaseClient.functions.invoke('send-voucher-email', {
                        body: { 
                            voucher_code: voucherData.voucher_code, 
                            payer_email: payerEmail,
                            validity_days: voucherData.validity_days 
                        }
                    });

                    if (emailSendError) {
                        // Log the error but don't throw, as email sending is secondary to voucher generation
                        console.error('Error invoking send-voucher-email Edge Function:', emailSendError);
                    } else {
                        console.log('send-voucher-email Edge Function invoked successfully:', emailSendData);
                    }
                } catch (emailError) {
                    console.error('Exception when trying to send voucher email:', emailError);
                }
            } else {
                console.warn('Cannot send voucher email: payerEmail is not available.');
            }
