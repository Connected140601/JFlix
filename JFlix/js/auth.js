...
async function checkUserSession() {
    ...
    supabase.auth.onAuthStateChange(async (event, session) => {
        console.log('[Auth Debug] onAuthStateChange event:', event, 'Session:', session, 'User:', session ? session.user : null);

        if (event === 'PASSWORD_RECOVERY') {
            console.log('[Auth] Password recovery event triggered.');
            // Potentially open a modal or show a message indicating that an email has been sent.
            // The actual password update will happen on the reset-password.html page.
            return; 
        } // Closes if (event === 'PASSWORD_RECOVERY')

        // Re-instated: Call handleAccessControl to manage page access based on auth state
        // It's important this runs on auth state changes to protect routes dynamically.
        if (await handleAccessControl(session ? session.user : null)) {
            return; // If redirected, stop further processing in this callback
        }
        ...
        if (event === 'SIGNED_IN' && session.user) {
            ...
            updateUIForLoggedOutUser();
        } else if (!session.user && event !== 'INITIAL_SESSION' && event !== 'USER_UPDATED' && event !== 'TOKEN_REFRESHED' && event !== 'PASSWORD_RECOVERY') {
...
function setupAuthEventListeners() {
    ...
    // Password Reset Form
    const passwordResetForm = document.getElementById('password-reset-form'); // This ID is in index.html for the forgot password modal's form
    if (passwordResetForm) {
        passwordResetForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            clearAuthMessage('password-reset-modal');
            const email = document.getElementById('reset-email').value; // Corrected ID for the email input in password-reset-modal
            const { data, error } = await supabase.auth.resetPasswordForEmail(email, {
                redirectTo: window.location.origin + '/reset-password.html', // URL to redirect to after email link click
            });
            if (error) {
