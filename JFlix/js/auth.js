...
window.isAuthModalActive = false; // Initialize flag for ad control
...
function setupAuthEventListeners() {
    ...
    document.querySelectorAll('.auth-modal .close-button').forEach(button => {
        button.addEventListener('click', () => {
            ...
            window.isAuthModalActive = false;
        });
    });
...
function openModal(modalId) {
    ...
    if (modal) {
        ...
        if (modalId === 'login-modal' || modalId === 'signup-modal') {
            window.isAuthModalActive = true;
        } else {
            window.isAuthModalActive = false; // For other modals like password reset
        }
    }
}
