...
async function handleAccessControl(user) {
    console.log(`[Auth] handleAccessControl called. User: ${user ? user.email : 'null'}, Path: ${window.location.pathname}`);
    const currentPath = window.location.pathname;
    const isNoAdsPath = currentPath.startsWith('/NOADS/') || currentPath === '/NOADS';

    const origin = window.location.origin;
    const mainSiteBaseUrl = origin + '/';
    const noAdsSiteBaseUrl = origin + '/NOADS/';
    const noAdsSiteHomepageUrl = noAdsSiteBaseUrl + 'index.html';

    let relativePath = "";
    if (isNoAdsPath) {
        // Strip /NOADS prefix to get the relative path
        // currentPath could be /NOADS/ or /NOADS/path/to/file.html
        relativePath = currentPath.substring('/NOADS'.length); // Removes /NOADS, leaves /path/to/file.html or /
        if (relativePath.startsWith('/')) {
            relativePath = relativePath.substring(1); // remove leading slash if present, e.g. path/to/file.html
        }
    } else {
        // currentPath is already relative to main site root (e.g. /path/to/file.html or /)
        if (currentPath.startsWith('/')) {
            relativePath = currentPath.substring(1);
        }
    }
    // Ensure relativePath is at least 'index.html' if it's empty (e.g. from '/NOADS/' or '/')
    if (relativePath === '' || relativePath === 'NOADS') { // Second check is for robustness
        relativePath = 'index.html';
    }

    let redirectToMain = mainSiteBaseUrl + relativePath;
    let redirectToNoAds = noAdsSiteBaseUrl + relativePath;
    ...
    if (user) { 
        ...
        if (premiumStatus.isPremium) {
            // If user is premium and on the main site (not a NOADS path)
            if (!isNoAdsPath) {
                // Redirect to NOADS homepage if not already there.
                // This covers the case after login if they were on a main site page.
                if (window.location.href !== noAdsSiteHomepageUrl) {
                    console.log('[Auth] Premium user on main site, redirecting to NOADS homepage.');
                    window.location.href = noAdsSiteHomepageUrl;
                    return true; // Indicate redirection happened
                }
            }
            // If premium user is already on a NOADS path, they stay.
        } else { // User is not premium (or status couldn't be determined without error, and it wasn't the critical error handled before)
            // If non-premium user is on NOADS site, redirect to main site's equivalent page.
            if (isNoAdsPath && window.location.href !== redirectToMain) {
                console.log('[Auth] Non-premium user on NOADS site, redirecting to main site.');
                window.location.href = redirectToMain;
                return true; 
            }
        }
    } else { // User is not logged in
        console.log(`[Auth] In handleAccessControl: User is null. Path: ${currentPath}, isNoAdsPath: ${isNoAdsPath}`);
        if (isNoAdsPath) {
            const mainSiteIndexUrl = mainSiteBaseUrl + 'index.html';
            console.log(`[Auth] Logged out user on NOADS site ('${currentPath}'), redirecting to main site INDEX ('${mainSiteIndexUrl}').`);
            window.location.href = mainSiteIndexUrl;
            return true; // Indicate redirection is attempted
        } else {
            console.log(`[Auth] In handleAccessControl: User is null, BUT NOT on a NOADS path. No redirect from here. Path: ${currentPath}`);
        }
    }
    return false; 
}
