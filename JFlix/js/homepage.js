...
async function loadTrendingContent() {
    const cacheKey = 'tmdb_trending_all_week';
  try {
    ...
    if (!trendingData) {
        // console.log('[Homepage] Trending content cache miss. Fetching from TMDB...');
        trendingData = await fetchFromTMDB('trending/all/week');
        if (trendingData && trendingData.results) { // Ensure data is valid before caching
            setCachedData(cacheKey, trendingData);
        } else {
            console.error('[Homepage] Failed to fetch valid trending data from TMDB or data format unexpected.');
            // trendingGrid.innerHTML = '<div class="error-message">Failed to load trending content. Please try again later.</div>'; // This is handled by the catch block
            // return; // Exit if data is invalid to prevent further errors
        }
    } else {
        // console.log('[Homepage] Trending content loaded from cache.');
    }
