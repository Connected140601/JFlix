...
const SERVER_URLS = {
  ...
  multiembed: 'https://multiembed.mov/' // Added Server 4
};
  ...
  if (type === 'movie' || type === MEDIA_TYPES.MOVIES) {
    ...
    switch(currentServer) {
      ...
      default:
        ...
        videoUrl = `${SERVER_URLS[currentServer]}movie?tmdb=${tmdbId}`;
        break;
      case 'multiembed': // Added Server 4 movie
        videoUrl = `${SERVER_URLS[currentServer]}?video_id=${tmdbId}&tmdb=1`;
        break;
    }
  } else {
    // TV show URL
    if (season !== null && episode !== null) {
      switch(currentServer) {
        ...
        default:
          ...
          videoUrl = `${SERVER_URLS[currentServer]}tv?tmdb=${tmdbId}&season=${season}&episode=${episode}`;
          break;
        case 'multiembed': // Added Server 4 TV show episode
          videoUrl = `${SERVER_URLS[currentServer]}?video_id=${tmdbId}&tmdb=1&s=${season}&e=${episode}`;
          break;
      }
    } else {
      ...
      switch(currentServer) {
        ...
        default:
          ...
          break;
        case 'multiembed': // Added Server 4 TV show (default S1E1 or full series)
            // Multiembed might require season/episode, defaulting to S1E1 if not specified for series view
            videoUrl = `${SERVER_URLS[currentServer]}?video_id=${tmdbId}&tmdb=1&s=1&e=1`; 
            break;
      }
    }
  }
