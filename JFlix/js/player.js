...
const SERVER_URLS = {
  ...
  multiembed: 'https://multiembed.mov/' // Base URL for Server 5
};
...
function loadVideo(id, type, season = null, episode = null) {
  ...
  if (type === 'movie' || type === MEDIA_TYPES.MOVIES) {
    ...
    switch(currentServer) {
      ...
      case 'multiembed':
        // Server 5: Multiembed
        videoUrl = `${SERVER_URLS.multiembed}?video_id=${id}&tmdb=1`;
        break;
      default:
    ...
    if (season !== null && episode !== null) {
      switch(currentServer) {
        ...
        case 'multiembed':
          // Server 5: Multiembed
          videoUrl = `${SERVER_URLS[currentServer]}?video_id=${id}&tmdb=1&s=${season}&e=${episode}`;
          break;
        default:
      ...
      switch(currentServer) {
        ...
        case 'vidsrcto':
          ...
          break;
        case 'multiembed':
          // Attempt to get current season/episode if available, otherwise, it might load season 1 episode 1 by default or show selector
          const currentSeason = mediaDetails.number_of_seasons > 0 ? (document.getElementById('season-selector') ? document.getElementById('season-selector').value : 1) : 1;
          const currentEpisode = 1; // Default to episode 1, actual episode selection handles specifics
          videoUrl = `${SERVER_URLS[currentServer]}?video_id=${id}&tmdb=1&s=${currentSeason}&e=${currentEpisode}`;
          break;
        default:
...
function updateServerNoticesVisibility() {
  ...
  // Correctly target the features div for multiembed
  ...
  // Correctly show/hide multiembed features
  if (multiembedFeatures) { 
    multiembedFeatures.style.display = currentServer === 'multiembed' ? 'block' : 'none';
  }
}
