const API_KEY = '6e86a46f4b05686fb4c1f3d1e07696f5';
...
function loadVideo(id, type, season = null, episode = null) {
  ...
  if (type === 'movie' || type === MEDIA_TYPES.MOVIES) {
    ...
    switch(currentServer) {
      ...
      case 'vidsrcto':
        ...
        break;
      default:
        videoUrl = `${SERVER_URLS['vidsrcxyz']}movie?tmdb=${tmdbId}`;
    }
  } else {
    // TV show URL
      ...
      switch(currentServer) {
        case 'vidsrcxyz':
          ...
          if (isAnime || type === MEDIA_TYPES.KOREAN || isCartoon) {
            // Use the specific episode format for anime, Korean, and cartoon content
        ...
        case 'videasy':
          ...
          // Add color parameter for different content types
          if (isAnime) {
            videoUrl += '&color=F43F5E'; // Red color for anime
          } else if (type === MEDIA_TYPES.KOREAN) {
            videoUrl += '&color=10B981'; // Green color for Korean
          } else {
            videoUrl += '&color=8B5CF6'; // Purple color for TV shows
          }
          break;
        case '2embed':
          // Server 3: 2embed.cc
          if (isAnime) {
            ...
            // Korean and cartoon shows use the same format as regular TV shows
        ...
        case 'vidsrcto':
          ...
          break;
        default:
          videoUrl = `${SERVER_URLS['vidsrcxyz']}tv?tmdb=${tmdbId}&season=${season}&episode=${episode}`;
      }
    } else {
      ...
      switch(currentServer) {
        ...
        case '2embed':
          if (isAnime) {
            ...
            // Korean and cartoon shows use the same format as regular TV shows but with specific episode
            videoUrl = `${SERVER_URLS[currentServer]}embedtv/${tmdbId}&s=1&e=1`;
          } else {
            // For regular TV shows, we can use the full season view
            videoUrl = `${SERVER_URLS[currentServer]}embedtvfull/${tmdbId}`;
          }
          break;
        case 'vidsrcto':
          if (isAnime || type === MEDIA_TYPES.KOREAN || isCartoon) {
            // Use specific episode format for anime, Korean, and cartoon content
            videoUrl = `${SERVER_URLS[currentServer]}tv/${tmdbId}/1/1`;
          } else {
        ...
        default:
          if (isAnime || type === MEDIA_TYPES.KOREAN || isCartoon) {
            videoUrl = `${SERVER_URLS['vidsrcxyz']}tv/${tmdbId}/1-1`;
          } else {
            videoUrl = `${SERVER_URLS['vidsrcxyz']}tv?tmdb=${tmdbId}`;
          }
      }
    }
  }
  ...
  videoPlayer.onerror = () => {
    videoMessage.innerHTML = `
      ...
      <p>Error loading video. Please try a different server.</p>
    `;
    videoMessage.style.display = 'flex';
    videoPlayer.style.display = 'none';
  };
...
function changeServer(server) {
  ...
  document.querySelectorAll('.server-button').forEach(button => {
    ...
    button.classList.remove('korean');
    button.classList.remove('movies');
    button.classList.remove('tvshows');
  });
  ...
  if (activeButton) {
    ...
    // Add media type class
    if (mediaType === MEDIA_TYPES.ANIME) {
      activeButton.classList.add('anime');
    } else if (mediaType === MEDIA_TYPES.KOREAN) {
      activeButton.classList.add('korean');
    } else if (mediaType === MEDIA_TYPES.CARTOON) {
      activeButton.classList.add('cartoon');
    } else if (mediaType === 'tv' || mediaType === MEDIA_TYPES.TV) {
      activeButton.classList.add('tvshows');
    } else if (mediaType === 'movie' || mediaType === MEDIA_TYPES.MOVIES) {
      activeButton.classList.add('movies');
    }
  }
  ...
  if (mediaType === 'tv' || mediaType === MEDIA_TYPES.TV || 
    ...
    if (activeEpisode) {
      const season = activeEpisode.getAttribute('data-season');
      const episode = activeEpisode.getAttribute('data-episode');
      ...
      if (videoMessage) {
        ...
        videoMessage.innerHTML = `
          ...
          <p>Loading ${mediaType === MEDIA_TYPES.ANIME ? 'anime' : 
             ...
             ${mediaType === MEDIA_TYPES.ANIME || mediaType === MEDIA_TYPES.CARTOON ? 'episode' : 'S' + season + ':E' + episode} from ${currentServer}...</p>
        `;
      }
      
      // Load the specific episode
      loadVideo(mediaId, mediaType, season, episode);
    } else {
      // Default to first episode if none is active
      loadVideo(mediaId, mediaType, 1, 1);
    }
  } else {
    // For movies, just load the video
...
async function init() {
  ...
  try {
    ...
    if (mediaType === 'tv' || mediaType === MEDIA_TYPES.TV || 
        mediaType === MEDIA_TYPES.ANIME || mediaType === MEDIA_TYPES.KOREAN) {
