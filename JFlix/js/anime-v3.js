...
async function searchTMDB() {
  ...
  try {
    ...
    const filteredResults = data.results.filter(item => 
      ...
      item.genre_ids && 
      item.genre_ids.includes(16) // Filter for anime genre
    );
...
function displaySearchResults(items) {
  ...
  items.forEach(item => {
    ...
    const year = item.media_type === 'movie' 
      ? (item.release_date ? item.release_date.substring(0, 4) : 'N/A') 
      : (item.first_air_date ? item.first_air_date.substring(0, 4) : 'N/A');
    const posterPath = item.poster_path ? `${IMAGE_BASE_URL}/w92${item.poster_path}` : 'images/poster-placeholder.png';
    ...
    `;
    resultItem.onclick = () => viewMediaDetails(item.id, item.media_type);
...
searchInput.addEventListener('input', () => {
  ...
  searchTimeout = setTimeout(searchTMDB, 300); // Debounce search
});
